...
// слайдов - 4, не 5. fslightbox считает количество всех ссылок с атрибутом data-fslightbox="gallery-olga" - исправляем.

document.getElementById("olga").addEventListener("click", e => {
  e.preventDefault();
  fsLightboxInstances["gallery-olga"].open(0); // открывает 1-й слайд
});

//для "gallery-victor" аналогично, нужно только свичнуть айдишник (#olga->#victor);id заранее выставляем в разметке html.
document.getElementById("victor").addEventListener("click", e => {
  e.preventDefault();
  fsLightboxInstances["gallery-victor"].open(0); // открывает 1-й слайд
});
